...
const CSHARP: Language = {
  ...
  firstSessionPrompt:
    ...
    'and note the workspace shape: file tree on the left, multi-tab editor, Run launches a real WPF window on their desktop, and a console-run button can run the active `.cs` file for non-GUI exercises. Send-to-tutor bundles the student note when present, relevant workspace files, and recent process output. ' +
...
const WEB: Language = {
  ...
  systemPromptIntro:
    ...
    'The student works in a multi-file Web project workspace with a Run button that starts a private Vite dev server and shows it in the live preview pane. They have a sidebar file tree, multiple tabs, and server logs/build errors beside the preview. ' +
    ...
    "The student has a 'Send to tutor' button next to the Run button. It can include an optional [NOTE] block with the student's specific question or confusion; answer that first when present. It auto-bundles these blocks for you: " +
    '[FILES] = open files plus likely editable workspace files auto-included from disk (open unsaved tabs take precedence and are marked "(unsaved)"); ' +
    ...
    'When the iframe is running, a PNG of the rendered page is also attached as a Claude vision content block (rasterised in the iframe via html-to-image). Use it alongside [DOM]: [DOM] is the structure (selectors, attributes, text content), the image is what the student actually sees (layout, fonts, colors, gradients, image positioning). The screenshot is a best-effort DOM-to-PNG rasterisation; it does NOT capture `<canvas>` element contents (renders blank), embedded `<video>` frames, `backdrop-filter`, custom shaders, or some animations — when the image disagrees with what you would expect from [DOM] / CSS, trust [DOM] and call out the discrepancy. ' +
    'If the message includes a `[SCREENSHOT]\\n(capture failed …)` note, no image is attached — proceed with [DOM] / [CONSOLE] / [SERVER]. ' +
